...
export default async function SheetsPage() {
  ...
  try {
    await dbConnect();
    const raw = await SheetModel.find({}).sort({ order: 1 }).lean();
    sheets = JSON.parse(JSON.stringify(raw));
  } catch {
    error = true;
  }
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="rounded-xl border border-border p-20 text-center">
          ...
          <p className="font-medium mb-1">Unable to load sheets</p>
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      ...
      {sheets.length === 0 ? (
        <div className="rounded-xl border border-border p-20 text-center">
          ...
          <p className="text-sm text-muted-foreground">No sheets found in the database yet.</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {sheets.map((sheet) => (
            <Link key={sheet._id} href={`/sheets/${sheet.slug}`} className="group">
              <Card className="h-full gap-0 py-0 hover:border-indigo-500/60 hover:shadow-md transition-all cursor-pointer">
                      ...
                      <div className="flex gap-1.5 flex-wrap justify-end">
                        {(sheet.tags as string[]).slice(0, 3).map((tag) => (
                          <Badge
                            key={tag}
                            variant="outline"
                            className={`text-xs ${tagClass(tag)}`}
                          >
                            {tag}
                          </Badge>
                        ))}
